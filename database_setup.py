...
def initialize_database():
    """Initializes the SQLite database with the banking schema."""
    conn = None
    try:
        ...
        conn.commit()
        print(f"Database '{DATABASE_FILE}' initialized successfully with '{SCHEMA_FILE}'.")
    except sqlite3.Error as e:
        print(f"Error initializing database: {e}")
    except FileNotFoundError:
        print(f"Error: Schema file '{SCHEMA_FILE}' not found. Please create it.")
    finally:
        if conn:
            conn.close()

def create_schema_file():
    """Creates a basic schema.sql file if it doesn't exist."""
...
""")
        print(f"Created a basic '{SCHEMA_FILE}' file.")
